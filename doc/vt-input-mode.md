...
`k e y b d` | Keyboard
`m o u s e` | Mouse
`f o c u s` | Window focus
`w i n s z` | Window size
`p a s t e` | Clipboard paste
`b r e a k` | Application break
...
This sequence fired after every key press and key release. Event data can contain a string generated by a keystroke as a set of codepoints: `C0 + ... + Cn`. The string can be fragmented and delivered by multiple consecutive events.
